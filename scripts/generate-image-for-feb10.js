/**
 * 2월 10일 인사이트에 Gemini API를 활용한 이미지 생성 및 적용
 */
...
async function generateImagesForFeb10() {
  console.log('🔄 2월 10일 인사이트에 Gemini API를 활용한 이미지 생성 시작...\n')
  ...
  const feb10Insights = allInsights?.filter(insight => {
  ...
  if (feb10Insights.length === 0) {
    console.log('⚠️  2월 10일자 발행된 인사이트가 없습니다.')
    return
  }

  console.log(`📋 발견된 인사이트: ${feb10Insights.length}개\n`)

  let successCount = 0
  let failCount = 0

  for (let i = 0; i < feb10Insights.length; i++) {
    const insight = feb10Insights[i]
    console.log(`[${i + 1}/${feb10Insights.length}] 인사이트 #${insight.id}: ${insight.title}`)
    ...
    // API 제한을 고려하여 지연
    if (i < feb10Insights.length - 1) {
      await new Promise(resolve => setTimeout(resolve, 1000)) // 1초 대기
    }
  }

  // Article도 업데이트 (첫 번째 인사이트의 이미지 사용)
  if (successCount > 0 && feb10Insights.length > 0) {
    const firstInsight = feb10Insights[0]
