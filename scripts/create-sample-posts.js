/**
 * 커뮤니티 활성화를 위한 샘플 게시글 생성 스크립트 (JavaScript 버전)
 * 실행 방법: node scripts/create-sample-posts.js
 */
...
async function createSamplePosts() {
  try {
    ...
    console.log(`\n총 ${samplePosts.length}개의 샘플 게시글을 생성합니다...\n`)
    ...
    for (let i = 0; i < samplePosts.length; i++) {
      ...
      if (error) {
        console.error(`✗ [${i + 1}/${samplePosts.length}] 실패: ${post.title}`)
        ...
        const boardTypeName = {
          ...
          'qna': 'Q&A',
          'tip': '팁 & 노하우'
        }[post.board_type] || post.board_type
        console.log(`✓ [${i + 1}/${samplePosts.length}] ${boardTypeName}: ${post.title}`)
    ...
    console.log('\n샘플 게시글 생성이 완료되었습니다!')
    console.log('커뮤니티 페이지에서 확인해보세요: /community')
  } catch (error) {
